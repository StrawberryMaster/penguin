...
oxfordLogoImg.src = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(oxfordSvgString)));
...
function drawAuthor(author) {
    ...
    const fontStr = getAuthorFontSize(author.length);
    ctxMain.font = fontStr;
...
function drawTemplate() {
    ...
    if (state.brand === BRAND_PENGUIN) {
        ...
        if ('letterSpacing' in ctxBg) {
            ctxBg.letterSpacing = (11 * 0.94) + 'px';
        }
        ctxBg.textAlign = 'right';
        ctxBg.fillText(txtPenguin, 220, bannerY + bannerHeight / 2); // 18px gap to the logo

        if ('letterSpacing' in ctxBg) {
            ctxBg.letterSpacing = (11 * 0.85) + 'px';
        }
        ctxBg.textAlign = 'left';
        ctxBg.fillText(txtClassics, 290, bannerY + bannerHeight / 2);
        ...
        if (penguinLogoImg.complete) {
            ...
            ctxBg.drawImage(
                penguinLogoImg,
                (config.canvas.width - logoWidth) / 2,
                bannerY + (bannerHeight - logoHeight) / 2,
                logoWidth,
                logoHeight
            );
        }
        ...
        if (oxfordLogoImg.complete) {
            ...
            const logoY = 672;
            const logoWidth = 255;
            const logoHeight = logoWidth * (24 / 555);
...
function switchBrand(brandName) {
    ...
    if (brandName === BRAND_PENGUIN) {
        state.isTemplateLoaded = true;
        drawTemplate();
    } else if (brandName === BRAND_OXFORD) {
        state.isTemplateLoaded = true;
        drawTemplate();
    } else {
